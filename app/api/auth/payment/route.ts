...
const PAYGATE_API_URL = process.env.PAYGATE_API_URL!;
const AUTH_TOKEN = process.env.PAYGATE_API_KEY!;

export async function POST(request: NextRequest) {
  try {
    const { amount, phoneNumber, network, identifier, description = 'Paiement sur site' } = await request.json();
    ...
    // Requête vers PayGate (équivalente au cURL fourni)
